...
const {IMAGE_DATA, getClient} = require('./compat');
...
const saveFansInfo = async infos => {
  let collection = client.db('db_bot').collection('cl_mabinogi_live_fans_v2')
...
const fetchTCData = (page = 1) => new Promise(resolve => {
  const url = 'https://evt08.tiancity.com/luoqi/2651032/home/index.php/lists';
...
const LiveAnalyzer = async(qq, group, content, callback) => {
  ...
  let allData = await client.db('db_bot').collection('cl_mabinogi_live_fans_v2').find({ season }).toArray()
...
const LiveInspect = async (qq, group, content, callback, auto = false) => {
  ...
  for(let i = 0; i < allList.length; i ++) {
    try{
      ...
      if(!auto) {
        const prevData = await client.db('db_bot').collection('cl_mabinogi_live_fans_v2').findOne({_id: `${roomId}_${prevHour}`, season})
