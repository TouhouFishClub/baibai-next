...
const { getClient } = require('../compat')
const MONGO_URL = require('../compat').mongourl;
...
const optionsetWhereCn = async ( optsNameCN, level ) => {
  ...
  collection = client.db('db_bot').collection('cl_mabinogi_optionset')
        ...
        await collection.updateOne(
          { '_id': `${optsNameCN}_${level}` },
          {
            '$set': {
              ...
              customWhere: context.split('\n').map(x => x.replace(/\r/g, '')).filter(x => x.trim()),
              level,
              author
            }
          }
